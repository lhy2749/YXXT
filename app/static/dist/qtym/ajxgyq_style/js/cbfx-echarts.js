$(function () {
    ...
    $("#end").attr({"value": dateend});
	 cbfxyqqs(datestart,dateend,ajbh);
	 cbfxmtfb(datestart,dateend,ajbh);
...
//传播分析舆情趋势
function cbfxyqqs(datestart,dateend,ajbh) {
...
//传播分析媒体分布
function cbfxmtfb(datestart,dateend,ajbh) {
...
//	var ajbh=$('#ajbh').val();
	...
	$.ajax({
		type : "post",
		async : false,    
		url : "cbfxmtfbAction",    //请求发送到mtfbServlet处
		data: {
	            "datestart": datestart,
	            "dateend": dateend,
	            "ajbh":ajbh
		...
		success : function(data) {
			...
			if (result) {
				for(var i=0;i<result.length;i++){
					var str = result[i].lx;
					lx.push(str.substring(0, 10));    //挨个取出类别并填入类别数组
				}
				for(var i=0;i<result.length;i++){       
					sum.push(result[i].sum);    //挨个取出销量并填入销量数组
				}
